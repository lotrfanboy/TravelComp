...
export default function OnboardingPage() {
  ...
  const onSubmit = async (data: OnboardingData) => {
    ...
    try {
      // Por enquanto, vamos apenas simular a atualização do perfil
      // Em uma implementação real, enviaríamos os dados para a API
      console.log("Perfil a ser atualizado:", {
        role: data.profileType,
        preferences: {
          travel: data.travelPreferences,
      ...
      setTimeout(() => {
        // Redirecionar para o dashboard específico com base no tipo de perfil
        if (data.profileType === 'nomad') {
          navigate("/nomad-dashboard");
        } else if (data.profileType === 'business') {
